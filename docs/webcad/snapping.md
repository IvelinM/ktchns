...
- **`getSnap(e, plane, skip)`** — the unified entry point:
  ...
  Returns `{ pos, type: 'endpoint'|'midpoint'|'origin'|'grid' }`.

- **`showSnap(pos, type)`** moves the `snapDot` sphere and colours it: origin magenta
  `0xff44dd`, endpoint cyan `0x00e5ff`, midpoint lime `0x76ff03`, grid white. The
  viewport `.snap-legend` mirrors these and shows whenever `mode !== 'idle'`.
